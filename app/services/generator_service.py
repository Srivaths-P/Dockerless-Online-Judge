...
async def generate_sample_testcase(
    ...
    if not problem.generator_code:
        ...
        log_user_event(user_id=current_user.id, user_email=current_user.email, event_type="generator_not_available",
                       details={"contest_id": contest_id, "problem_id": problem_id})
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail="Test case generator not available for this problem.")
    ...
    try:
        ...
        log_user_event(user_id=current_user.id, user_email=current_user.email, event_type=f"generator_result_{generator_result.get('status', 'unknown')}",
                       details={
        ...
        log_user_event(user_id=current_user.id, user_email=current_user.email, event_type="generator_http_exception",
                       details={"contest_id": contest_id, "problem_id": problem_id, "detail": e.detail, "status_code": e.status_code})
        raise e
        ...
        traceback.print_exc()
        raise HTTPException(status_code=status.HTTP_500_INTERNAL_SERVER_ERROR, detail="Failed to run test case generator.") from e